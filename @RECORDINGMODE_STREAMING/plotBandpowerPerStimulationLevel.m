...
if numel(recording_indx) == 1
    lbl_recording = "Recording " + string(recording_indx);
else
    lbl_recording = "All Recordings";
end
...
for r = recording_indx
    ...
    while numel(unique_channel_names) > numel(unique_titration_levels)
        unique_titration_levels{end+1} = [];
    end
    ...
    for channel = 1:num_channels
        ...
        L = length(data);
        amp = mode(data);
        while sum(data == amp) >= 20 % We defined that each level should be at least 10 seconds
            steps(end+1) = amp;
            data(data == amp) = [];
            amp = mode(data);
        end
        ...
        indx_channel = find(strcmp(unique_channel_names, obj.streaming_parameters.time_domain.channel_names{r}{channel}));
        unique_titration_levels{indx_channel} = unique([unique_titration_levels{indx_channel}, steps]);

        titration_levels{r}{channel} = steps;

        stim_data = round([streaming_stim_data(:, channel); -1.0], 1); % Add final value
        level_start = find(ismember(stim_data, round(steps,1)),1,'first');
        stim_data = stim_data(level_start:end);
        level_start = 1;
        level_end = 0;
        for i = 1:numel(steps)
...
nChannels = numel(unique_channel_names);
median_lower_stimulation = zeros(1,nChannels);
msg = [];
for channel = 1:nChannels

    % Get lowest stimulation median
    channel_table = epochs_power_in_bands(strcmp(epochs_power_in_bands.("Channel"), unique_channel_names{channel}), :);
    lower_level_table = channel_table(ismember(channel_table.("Stimulation Amplitude"), unique_titration_levels{channel}(1)),:);
    median_lower_stimulation(channel) = median(lower_level_table.("Power"));

    % Get stimulation amplitude ticks
    amp_ticks = intersect( channel_table.("Stimulation Amplitude"), unique_titration_levels{channel} );
    
    if numel(amp_ticks) < 2
        msg = [msg; string(unique_channel_names{channel})];
    else
        
        figure('Position', [50, 200, 600, 400]);
        set(gcf,'color','w');
        % Get highest whisker (to accomodate for statistics)
        max_whisker = 0;
        for level = 1:numel(amp_ticks)
            aux_table = channel_table(channel_table.("Stimulation Amplitude") == amp_ticks(level), :);
            q1 = quantile(aux_table.Power, 0.25);
            q3 = quantile(aux_table.Power, 0.75);
            iqr = q3 - q1;
            upper_whisker = min(max(aux_table.Power), q3 + 1.5 * iqr);
            if upper_whisker > max_whisker
                max_whisker = upper_whisker;
            end
        end
        aux_table = channel_table;
        ...
        title(unique_channel_names{channel} + " - " + lbl_recording + sprintf("\n%s: %d-%d Hz", data_type, bounds));
...
if ~isempty(msg)
    msgbox([sprintf("The following channels do not have 2 or more stimulation amplitude levels in %s:", lower(lbl_recording)); msg]);
end
